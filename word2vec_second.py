...
def pre_process_tweets(url):
    ...
    for i in range(len(tweet_df)):
        ...
        # print('Dataframe: ', tweet_df['text'][i])

        filtered_tweet = bagOfWords(tweet_df['text'][i])
        ...
        filtered_tweet.append(str(tweet_df['id'][i]))
        list_of_tweets.append(filtered_tweet)
        # list_of_tweets.append([filtered_tweet, tweet_df['id'][i]])
    f.close()

    # df = pd.DataFrame(list_of_tweets)
    # df.to_csv('Tweet_id.csv', mode='a', header=False)
...
def getDistance(tweet1, tweet2):
    ...
    #Because last item in tweets is id
    tweet1 = tweet1[:-1]
    tweet2 = tweet2[:-1]
